    ...
    <div className="fixed inset-0 flex" style={{ backgroundColor: "var(--background)" }}>
      ...
      <div
        ...
        <div className="px-4 pb-2 shrink-0">
          <div className="flex items-center rounded-lg p-0.5" style={{ backgroundColor: "var(--surface-hover)" }}>
            <button onClick={() => setResponseMode("normal")}
              className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition-all"
              style={{ color: responseMode === "normal" ? "var(--primary)" : "var(--text-tertiary)", backgroundColor: responseMode === "normal" ? "var(--surface)" : "transparent" }}>
              {responseMode === "normal" && (
                <span className="absolute inset-0 rounded-md" style={{ boxShadow: "0 1px 4px rgba(16,163,127,0.15)" }} />
              )}
            ...
            <button onClick={() => setResponseMode("deep")}
              className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-md text-xs font-medium transition-all relative"
              style={{ color: responseMode === "deep" ? "#a78bfa" : "var(--text-tertiary)", backgroundColor: responseMode === "deep" ? "var(--surface)" : "transparent" }}>
              <svg className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" />
              </svg>
              Pensar
