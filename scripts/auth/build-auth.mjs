...
/* SWAP de lados (SOLO escritorio): el form va a la IZQUIERDA y la marca (oscura)
   a la DERECHA. En el DOM el form va primero (mejor para lectores de pantalla).
   OJO: con min-width para NO pisar el apilado de móvil (@media max-width:900 del
   export pone 1 columna) — si va sin media, en móvil quedan 2 columnas apretadas. */
...
/* Sin spinner: el form de Clerk aparece con un fade suave (la animación va en
   rootBox vía appearance) — no se siente "cargando", solo aparece. */
@keyframes av-in { from { opacity: 0; transform: translateY(6px); } to { opacity: 1; transform: none; } }
@media (prefers-reduced-motion: reduce) { .av .cl-rootBox { animation: none !important; } }

/* "Secured by Clerk": reposicionado al pie del formulario, separado del link de
   registro, centrado y sutil. (Reposicionar está permitido; ocultarlo no.) */
.av .cl-footer { align-items: center; }
.av .cl-footerItem { margin-top: 20px; opacity: .6; }
