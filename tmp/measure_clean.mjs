...
// Capture screenshots AND measure at multiple points
const targets = [50, 150, 300, 500, 800, 1200, 1800, 2500];
for (const target of targets) {
  const wait = target - (Date.now() - startTime);
  if (wait > 0) await page.waitForTimeout(wait);
  
  await page.screenshot({ path: `C:/Users/joses/Documents/mente-ai/tmp/clean_${target}ms.png`, fullPage: false });
  ...
  console.log(`t=${Date.now() - startTime}ms: hero="${info.heroText}", heroOpacity=${info.heroOpacity}, sidebarWidth=${info.sidebarWidth}`);
}
